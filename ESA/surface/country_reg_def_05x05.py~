...
""" this module is used to define the sub regions based on Transcomm definition
...
class country_map:
    ...
    def __init__(self, sel_country, cn_map):
        ...
        nf=NetCDFFile(cn_map, 'r')
        dig_map=nf.variables['map']
        dig_map=array(dig_map)
        ...
        cn_id_list=array(cn_id_list)
        cn_id=cn_id_list[ientry]
        dig_map=dig_map.astype(integer)
        print 'cn_id', ientry, cn_id
        
        
        sel_cells=where(dig_map==cn_id)
        sel_cells=squeeze(sel_cells)
        idx_lon=sel_cells[0]
        idx_lat=sel_cells[1]
        ...
        map_lon=array(map_lon)
        map_lat=nf.variables['latitude']
        map_lat=array(map_lat)
        nf.close()
    ...
    def divid_country(self, xst, xend, yst, yend, dx, dy, in_map=None):
        ...
        ix=argmin(abs(cmap2.lon-xst))
        wnd_xst=cmap2.lon[ix]
        idx=argmin(abs(cmap2.lon-xend))
        wnd_xend=cmap2.lon[ix]

        # x coordinate
        wnd_x=arange(wnd_xst, wnd_xend+dx, dx)
        
    
        ix=argmin(abs(cmap2.lat-yst))
        wnd_yst=cmap2.lat[ix]
        idx=argmin(abs(cmap2.lat-yend))
        wnd_yend=cmap2.lon[ix]


        # y coordinate
        
        wnd_y=arange(wnd_yst, wnd_yend+dy, dy)
        ...
        for ic in range(ncell):
            if ((x[ic]>=wnd_xst) & (x[ic]<=wnd_xend) & (y[ic]>=wnd_yst) & (y[ic]<=wnd_yend)):                
                ix=argmin(abs(wnd_x-x[ic]))
                iy=argmin(abs(wnd_y-y[ic]))
