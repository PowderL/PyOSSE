...
! added by lf to read in ensemble menu 

         ELSE IF ( INDEX( LINE, 'ENSEMBLE MENU'   ) > 0 ) THEN 
            CALL READ_ENSEMBLE_MENU
...
!      
      ...
      FUNCTION READ_ONE_LINE2( EOF, LOCATION ) RESULT( LINE )
!
!******************************************************************************
!  Subroutine READ_ONE_LINE reads a line from the input file.  If the global 
!  variable VERBOSE is set, the line will be printed to stdout.  READ_ONE_LINE
...
! added by lf to read in the ensemble menu 

      SUBROUTINE SPLIT_ONE_LINE2( SUBSTRS, N_SUBSTRS, N_EXP, LOCATION ) 
!
!******************************************************************************
!  Subroutine SPLIT_ONE_LINE reads a line from the input file (via routine 
!  READ_ONE_LINE), and separates it into substrings. (bmy, 7/20/04)
...
!
      ...
      ! Arguments
      CHARACTER(LEN=600), INTENT(OUT) :: SUBSTRS(MAXDIM)
      INTEGER,            INTENT(OUT) :: N_SUBSTRS
      INTEGER,            INTENT(IN)  :: N_EXP
      CHARACTER(LEN=*),   INTENT(IN)  :: LOCATION 
...
C pip
 ...
 100     FORMAT( 'Expected ',i2, ' substrs but found ',i3 )
      ...
      ! Return to calling program
      END SUBROUTINE SPLIT_ONE_LINE2
...
!
      ...
      USE SATETRACK_MOD,   ONLY : SET_SATETRACK
...
#     include "CMN_DIAG"  ! ND40
      ...
      ! by lf to add satetrack

      CHARACTER(LEN=255) :: SIFILE
      CHARACTER(LEN=255) :: SOFILE
      ! end by lf
      ...
      ! by lf Input file name (satetrack data points)
      CALL SPLIT_ONE_LINE( SUBSTRS, N, 1,  'read_planeflight_menu:4' )
      READ( SUBSTRS(1:N), '(a)' ) SIFILE

      ! Output sate track file name
      CALL SPLIT_ONE_LINE( SUBSTRS, N, 1,  'read_planeflight_menu:5' )
      READ( SUBSTRS(1:N), '(a)' ) SOFILE
      !=================================================================
      ! Print to screen
      !=================================================================
...
!
      ...
      CHARACTER(LEN=255)  :: FILE
 ! lf 
      INTEGER             :: ST_TRACER, END_TRACER, IL
      CHARACTER(LEN=600)  :: SUBSTRS2(MAXDIM)      
      ...
      print *, FILE
      ! Tracers to include
      CALL SPLIT_ONE_LINE2( SUBSTRS2, N_ND50, -1, 'read_nd50_menu:3' )
      print  *, N_ND50
...
! DO N = 1, N_ND50
      ...
      READ( SUBSTRS2(1), * ) ST_TRACER
      READ( SUBSTRS2(2), * ) END_TRACER
      print *, ST_TRACER, END_TRACER
      IL=1
C       PRINT *, 'TRACERS IN DIAG51' 
      DO N=ST_TRACER, END_TRACER
         TRACERS(IL)=N
         PRINT *, IL, TRACERS(IL)
         IL=IL+1
      END DO
...
#     include "CMN_DIAG"   ! NDxx flags
      ...
      LOGICAL             :: DO_ND51
      INTEGER             :: N,      I,       AS
      INTEGER             :: N_ND51, FREQ,    TRACERS(210)
      ...
      ! Tracers to include
      ! lf CALL SPLIT_ONE_LINE( SUBSTRS, N_ND51, -1, 'read_nd51_menu:3' )

      ! lf DO N = 1, N_ND51
      ! lf    READ( SUBSTRS(N), * ) TRACERS(N)
      ! lf ENDDO

      ! Tracers to include
      CALL SPLIT_ONE_LINE2( SUBSTRS2, N_ND51, -1, 'read_nd51_menu:3' )
c      print*, substrs2(1)
c      stop
c      print*, '--',do_nd51
c      call flush(6)
...
!------------------------------------------------------------------------------
      SUBROUTINE READ_ENSEMBLE_MENU
!     
!******************************************************************************
!  Subroutine READ_ENSEMBLE_MENU reads the ENSEMBLE  MENU section of the GEOS-CHEM 
!  input file. (lf, 7/20/08)
!
!  NOTES:
!******************************************************************************
!
      ! References to F90 modules
      USE ERROR_MOD,  ONLY : ERROR_STOP
      USE CO2_MOD,    ONLY : INIT_CO2_ENKF
      ...
      INTEGER            :: ENR_PBSTART
      INTEGER            :: DOBKRUN
      INTEGER            :: ENR_YEARS(220), ENR_DOYS(220),
     $     ENR_PB_USES(220)
      ...
      ENDIF
      PRINT *, 'I AM HERE'
      ! READ START POSITION REG 
      CALL SPLIT_ONE_LINE( SUBSTRS, N, 1, 'read_ensemble_menu:1' )
      READ( SUBSTRS(1), * ) MEMBSTART
!     READ START POSITION REG 
      CALL SPLIT_ONE_LINE( SUBSTRS, N, 1, 'read_ensemble_menu:2' )
      READ( SUBSTRS(1:N), * ) MEMBEND
!      PRINT *, MEMBEND

!     READ START JOB TAGGED FIRST OR NOT 
      CALL SPLIT_ONE_LINE( SUBSTRS, N, 1, 'read_ensemble_menu:2' )
      READ( SUBSTRS(1:N), * ) DOBKRUN
      
      CALL SPLIT_ONE_LINE( SUBSTRS, N, 1, 'read_ensemble_menu:3' )
      READ( SUBSTRS(1:N), * ) ENR_NTIME
!      PRINT *, ENR_NTIME
      CALL SPLIT_ONE_LINE2( SUBSTRS2, ENR_NTIME, -1, 
     $ 'read_ensemble_menu:4' )
      DO N = 1, ENR_NTIME
         READ( SUBSTRS2(N), * ) ENR_DOYS(N)
!         print*, ENR_DOYS(N), ENR_NTIME
      ENDDO
      
      CALL SPLIT_ONE_LINE2( SUBSTRS2, ENR_NTIME, -1, 
     $ 'read_ensemble_menu:5' )
      DO N = 1, ENR_NTIME
         READ( SUBSTRS2(N), * ) ENR_YEARS(N)
!     print*, ENR_YEARS(N), ENR_NTIME
      ENDDO
      CALL SPLIT_ONE_LINE( SUBSTRS, N, 1, 'read_ensemble_menu:6' )
      READ( SUBSTRS(1:N), * ) ENR_PBSTART
      
      CALL SPLIT_ONE_LINE2( SUBSTRS2, ENR_NTIME, -1, 
     $     'read_ensemble_menu:7' )
      DO N = 1, ENR_NTIME
         READ( SUBSTRS2(N), * ) ENR_PB_USES(N)
!     print*, ENR_YEARS(N), ENR_NTIME
      ENDDO
      
      ! READ END POSITION REG 
      CALL SPLIT_ONE_LINE( SUBSTRS, N, 1, 'read_ensemble_menu:6' )
      READ( SUBSTRS(1:N), '(a)' ) FILE
      ...
      CALL SPLIT_ONE_LINE( SUBSTRS, N, 1, 'read_unix_cmds_menu:7' )
      ! Return to calling program
      CALL INIT_CO2_ENKF(ENR_YEARS(1:ENR_NTIME), ENR_DOYS(1:ENR_NTIME), 
     &     ENR_PB_USES(1:ENR_NTIME), 
     &     ENR_NTIME, MEMBSTART, MEMBEND, ENR_PBSTART, 
     &     TRIM(FILE), DOBKRUN)

      
      END SUBROUTINE READ_ENSEMBLE_MENU
