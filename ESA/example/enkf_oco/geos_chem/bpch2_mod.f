...
#     include "define.h" 
 ...
 100        FORMAT( 'READ_BPCH2: Found data for TAU = ', f10.2, 
     &              ' and tracer # ', i6 )
      ...
      SUBROUTINE READ_BPCH2_CO2_FLUX( FILENAME, 
     &     FLUX_ST,       TRACER_ST, 
     &     TRACER_END, 
     &     IX, JX, LX, ARRAY ) 
      USE ERROR_MOD, ONLY : ERROR_STOP
...
#     include "define.h" 
      ...
      REAL*8,            INTENT(OUT) :: ARRAY(IX, JX, LX)      
...
c      call flush(6)
     ...
     &     MODELNAME, LONRES, LATRES, HALFPOLAR, CENTER180
      ...
      IF ( IOS /= 0 ) CALL IOERROR( IOS, IU_FILE, 
     &     'read_bpch2_co2_flux:1' )
     ...
     &     NSKIP
      ...
      IF ( IOS /= 0 ) CALL IOERROR( IOS, IU_FILE, 
     &     'read_bpch2_co2_flux:2' )
      
      READ( IU_FILE, IOSTAT=IOS ) 
     &     ( ( ( TEMPARRAY(I,J,L), I=1,NI ), J=1,NJ ), L=1,NL )
      
      IF ( IOS /= 0 ) CALL IOERROR( IOS, IU_FILE, 
     &     'read_bpch2_co2_flux:3' )
      ...
      SUBROUTINE READ_BPCH2_BF( FILENAME, 
     &     IX, JX, LX, TAU_IN, ARRAY,
     &     NBF)
...
c      call flush(6)
     ...
     &        'read_bpch2_bf:2' )
         ...
         IF ((TAU_IN>=ZTAU0).AND.(TAU_IN<ZTAU1)) THEN  
            READ( IU_FILE, IOSTAT=IOS ) 
     &           ( ( ( TEMPARRAY(I,J,L), I=1,NI ), J=1,NJ ), L=1,NL )
     ...
     &           'read_bpch2_bf:3' )
      ...
      ! Return to calling program
      END SUBROUTINE READ_BPCH2_BF


      SUBROUTINE READ_BPCH2_PB_FLUX( FILENAME, 
     &     IX, JX, LX, ARRAY,
     &     TRACER_END)
...
c      call flush(6)
     ...
     &     'read_bpch2_co2_flux:3' )
      ...
      ! Return to calling program
      END SUBROUTINE READ_BPCH2_PB_FLUX


      SUBROUTINE READ_BPCH2_REG_FLUX( FILENAME, IX, JX, LX, ARRAY ) 
      USE ERROR_MOD, ONLY : ERROR_STOP
      USE FILE_MOD,  ONLY : IU_FILE, IOERROR
...
c      call flush(6)
     ...
     &     NSKIP
      ...
      IF ( IOS /= 0 ) CALL IOERROR( IOS, IU_FILE, 
     &     'read_bpch2_reg_flux:2' )
      
      READ( IU_FILE, IOSTAT=IOS ) 
     &     ( ( ( TEMPARRAY(I,J,L), I=1,NI ), J=1,NJ ), L=1,NL )
      
      IF ( IOS /= 0 ) CALL IOERROR( IOS, IU_FILE, 
     &     'read_bpch2_reg_flux:3' )
      ...
      ! Return to calling program
      END SUBROUTINE READ_BPCH2_REG_FLUX
