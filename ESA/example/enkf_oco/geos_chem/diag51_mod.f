...
!
      ...
      INTEGER              :: I0,             J0
      INTEGER              :: ND51_N_TRACERS, ND51_TRACERS(210)
      ...
      !
      !! For timeseries between 1210 and 1700 LT, uncomment this code:
      !!
...
C Compute the mixing ratio of water vapour
      ALLOCATE( AVGW( IIPAR, JJPAR, LLPAR ), STAT=AS )
      IF ( AS /= 0 ) CALL ALLOC_ERR( 'AVGW in DIAG51' )
      AVGW_ALLOCATED = ALLOCATED( AVGW )
C Compute AVGW, the mixing ratio of water vapor
      CALL MAKE_AVGW
      print *, 'ND51_N_TRACERS', ND51_N_TRACERS
...
!$OMP+SCHEDULE( DYNAMIC )
     ...
     &              ( AIRDEN(L,I,J) * XNUMOLAIR * 1d-6 * GOOD(I) )
            ...
            ! lf ELSE IF ( N == 80 .and. IS_OPTD ) THEN
            ELSE IF ( N == 200 .and. IS_OPTD ) THEN  
               !--------------------------------------
               ! COLUMN OPTICAL DEPTH [unitless]
               !--------------------------------------
               Q(X,Y,1,W) = Q(X,Y,1,W) + ( OPTD(L,I,J) * GOOD(I) )

            ! lf ELSE IF ( N == 81 .and. IS_CLDTOPS ) THEN
            ELSE IF ( N == 201 .and. IS_CLDTOPS ) THEN   
               !--------------------------------------
     ...
     &                         ( ODAER(I,J,L,H) * SCALE400nm * GOOD(I) )
            ...
            ! lf ELSE IF ( N == 87 .and. IS_CHEM ) THEN               
            ELSE IF ( N == 202 .and. IS_CHEM ) THEN
     ...
     &                 ( AVGW(I,J,L) * GOOD(I) )
               ELSE
                  PRINT*,'---AVGW NOT ALLOCATED IN DIAG51_MOD---'
                  CALL FLUSH(6)
                  STOP
               ENDIF
...
!$OMP+PRIVATE( X, Y, K, W )
      ...
      DO W = 1, ND51_N_TRACERS
         ...
         SELECT CASE( ND51_TRACERS(W) )
                                ...
                                !   CASE( 89, 90, 74, 75 )
         ! lf 2010.08.19, changed to do nothing for these tracers
      CASE( -89, -90, -74, -75 )
...
#     include "CMN_SIZE"   ! Size parameters
      ...
      INTEGER,            INTENT(IN) :: N_ND51, TRACERS(210)
